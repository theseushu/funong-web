...
const stateSelector = (method) => (state, { type, publish: { objectId } }) => ({ ...selectors[type][method](state)[objectId] });
const bind = (method) => (dispatch, { type, publish: { objectId } }) => bindActionCreators({ onClick: () => actions[type][method]({ objectId, meta: { storeKey: objectId } }) }, dispatch);
const selectStates = ({ pending }, { onClick }, { type, publish, ...props }) => ({ // eslint-disable-line
  pending,
  onClick,
  ...props,
});
