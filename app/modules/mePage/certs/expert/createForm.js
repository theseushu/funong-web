...
export default (cert) => reduxForm({
  ...
  initialValues: cert || { desc: '', images: [] },
})(connect(
  (state) => ({ cert: expert(state) }),
  (dispatch) => ({
    onSubmit: ({ desc, images }) => new Promise((resolve, reject) => {
      const params = { type: certTypes.expert, desc, images, meta: { resolve, reject: (err) => reject(new SubmissionError({ _error: { code: err.code, message: err.message } })) } };
      if (cert) {
