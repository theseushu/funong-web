...
export const ButtonSendSmsCode = ({ onClick, phone, text, accent = false, disabled }) => (
  <Button
    raised
    accent={accent}
    onClick={(e) => {
...
export const CountDownButton = ({ remains, phone, onClick, disabled }) => {
  ...
  return <ButtonSendSmsCode disabled={disabled} phone={phone} onClick={onClick} text="发送验证码" />;
};
...
const RequestSmsCodeButton = ({ pending, fulfilled, time = 0, rejected, onClick, phone, disabled }) => {
  if (pending) { // put disabled at last so it can override 'disabled' in props
    ...
    return <ButtonSendSmsCode disabled={disabled} text="发送失败,请重试" phone={phone} onClick={onClick} accent></ButtonSendSmsCode>;
  ...
  return <ButtonSendSmsCode disabled={disabled} onClick={onClick} phone={phone} text="发送验证码" />;
};
