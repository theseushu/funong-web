...
const debug = require('debug')('funong-web:api/utils/createCompositeDucks');
...
export const createSaga = ({ apiName, actionConstant, stateActionConstant }, beforeFulfilledSaga, apiSaga) => {
  const actionSaga = function* ({ payload, meta: { storeKey, resolve, reject } }, api) {
    ...
    try {
      ...
      const result = apiSaga ? yield* apiSaga(api, payload, { storeKey }) : yield call(_get(api, (apiName)), payload);
      if (beforeFulfilledSaga) {
        const beforeResult = yield* beforeFulfilledSaga(result, { payload, meta: { storeKey, resolve, reject } });
